"""
ETSformer Encoder-Decoder layers (ICLR 2023)
Paper: https://arxiv.org/abs/2202.01381

Core components:
    - ExponentialSmoothing: learnable alpha-based smoothing via FFT convolution
    - GrowthLayer: damped trend extraction via exponential smoothing
    - FourierLayer: seasonal pattern extraction via top-k frequency selection
    - LevelLayer: level component with auxiliary growth/season inputs
    - Transform: training-time data augmentation (jitter + scale + shift)
"""
...
class FourierLayer(nn.Module):
    ...
    def topk_freq(self, x_freq):
        values, indices = torch.topk(x_freq.abs(), self.k, dim=1,
        ...
        index_tuple = (mesh_a.unsqueeze(1).to(indices.device),
                       indices,
...
class EncoderLayer(nn.Module):
    def __init__(self, d_model, nhead, c_out, seq_len, pred_len, k,
                 dim_feedforward=None, dropout=0.1, activation='sigmoid',
                 layer_norm_eps=1e-5):
        super().__init__()
        self.d_model = d_model
        self.nhead = nhead
