...
class Dataset_ETT_hour(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 ...
                 num_coarse=2, num_fine=4): # [Fix] 加回参数以兼容 data_factory
        if size == None:
            self.seq_len = 24 * 4 * 4
            self.label_len = 24 * 4
            self.pred_len = 24 * 4
    ...
    def __read_data__(self):
        ...
        train_data_slice = df_data[border1s[0]:border2s[0]]
        self.scaler.fit(train_data_slice.values)
...
class Dataset_ETT_minute(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 ...
                 num_coarse=2, num_fine=4): # [Fix] 加回参数
        if size == None:
            self.seq_len = 24 * 4 * 4
            self.label_len = 24 * 4
            self.pred_len = 24 * 4
...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        # [Bug Fix] train_only 逻辑修复：避免 num_vali 变为负数
        if self.train_only:
